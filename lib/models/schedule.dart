    ...
    if (result.length == 1) {
      for (var row in result) {
        return Schedule(
          ...
          startTime: DateFormat("hh:mm").parse(row[4].toString()).subtract(const Duration(hours: 6)),
          endTime: DateFormat("hh:mm").parse(row[5].toString()).subtract(const Duration(hours: 6)),
        );
      }
    }
    throw Exception(
  ...
  Future<void> add() async {
    if (validate() && !await exist(id)) {
      await DatabaseService.to.connection.query(
        ...
        VALUES (?,?,?,?,?,?)
        ''',
        [id, course!.id, classroom!.id, weekDay.index, startTime.toUtc(), endTime.toUtc()],
      );
    } else {
