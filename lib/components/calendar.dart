...
class Calendar extends GetView<TheCalendarController> {
  ...
  Widget build(BuildContext context) {
    ...
    return CalendarControllerProvider( // ¯\_(ツ)_/¯ el package funciona asi
      controller: EventController()..addAll(controller.events),
      child: WeekView(
        eventTileBuilder: (date, events, boundry, start, end) {
          // Metanle bustra
          return Container(constraints: const BoxConstraints(maxHeight: 55), child: WorthyText.bodySmall(events[0].title, style: const TextStyle(overflow: TextOverflow.fade),),);
      },
      showLiveTimeLineInAllDays: true,
      width: 390, // que sea dinamico
