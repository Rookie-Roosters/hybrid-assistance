...
class ListController extends GetxController with StateMixin {
  ...
  void getCode() async {
    change(null, status: RxStatus.loading());
    code = await StudentAttendance.generateCodeNList(selectedClass.id, selectedClass.course!);
  ...
  bool checkTime() {
    if ((DateTime.now().hour == selectedClass.startTime.hour) ||
        ((DateTime.now().hour + 1 == selectedClass.endTime.hour) && (DateTime.now().minute < selectedClass.endTime.minute + 2))) {
      return true;
    } else {
      return false;
    }
